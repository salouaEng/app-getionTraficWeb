...
import { OPERATING_SYSTEMS } from "../../utils/index";

const OperatingSysSelector = ({
    id,
    open,
    disabled = false,
    onToggle,
    ...
    }, [ref, open, onToggle, selectedOperatingSystems]);

    const handleSelectAll = () => {
        setSelectedOperatingSystems(OPERATING_SYSTEMS.map((os) => os.id));
    };
    ...
    const handleSelectOperatingSystem = (osId) => {
        if (selectedOperatingSystems.includes(osId)) {
            setSelectedOperatingSystems(selectedOperatingSystems.filter((id) => id !== osId));
        } else {
            setSelectedOperatingSystems([...selectedOperatingSystems, osId]);
        }
    };
            ...
            <div className="mt-1 relative">
                ...
                >
                    <span className="truncate flex items-center">
                        {selectedOperatingSystems.length === 0 ? (
                            ...
                            <span>{OPERATING_SYSTEMS.find((os) => os.id === selectedOperatingSystems[0]).name}</span>
                        ) : (
                            <span>
                                {OPERATING_SYSTEMS.find((os) => os.id === selectedOperatingSystems[0]).name} and{" "}
                                {selectedOperatingSystems.length - 1} more
                            </span>
                        )}
                    </span>
                    <span
                ...
                {open && (
                    ...
                    >
                        ...
                        >
                            {OPERATING_SYSTEMS.filter((os) =>
                                os.name.toLowerCase().startsWith(query.toLowerCase())
                            ).length === 0 ? (
                                ...
                                OPERATING_SYSTEMS.filter((os) =>
                                    ...
                                    return (
                                        ...
                                        key={`${value.id}-${index}`}
                                            ...
                                            role="option"
                                            onClick={() => {
                                                handleSelectOperatingSystem(value.id);
                                                onChange(value.id);
                                                setQuery("");
                                                onToggle();
                                            }}
                                        >
                                            ...
                                            {selectedOperatingSystems.includes(value.id) ? (
...
export default OperatingSysSelector;
